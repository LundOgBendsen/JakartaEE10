...
public class IngredientTxServlet extends HttpServlet
{
  ...
  protected void processRequest(@SuppressWarnings("unused") final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException
  {
    ...
    try (PrintWriter out = response.getWriter())
    {
      ...
      out.println("</head>");
      out.println("<body>");
      out.println("<h1>CSV-to-Database Ingredient Tx Job</h1>");
      ...
      try
      {
        ...
        tx.commit();
        out.println("Ingredient Tx Job was submitted just now: " + jid + "<br>");

      }
      catch (Exception e)
      {
        out.println("Ingredient Tx Job could not be submitted<br>");

      }

      out.println("<br><br>Check server.log for output<br>Look at \"IngredientTxJob.xml\" for Job XML.");
      out.println("</body>");
      out.println("</html>");
    }
    catch (JobStartException | JobSecurityException ex)
    {
      Logger.getLogger(IngredientTxServlet.class.getName()).log(Level.SEVERE, null, ex);
    }
  }
