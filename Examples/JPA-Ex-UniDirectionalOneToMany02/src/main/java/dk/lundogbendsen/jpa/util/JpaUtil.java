...
public final class JpaUtil {
	...
	public static void configureHibernateLogging(HibernateLoggingConfig config) {
		...
		if (config.logDdlEnabled) {
			Logger.getLogger("org.hibernate.tool.hbm2ddl").setLevel(Level.FINE);
		} else {
	...
	private static String toPrettyString(int tabCount, @SuppressWarnings("rawtypes") Collection collection, boolean printArrayAsList) {
		String s = "[";
		tabCount++;
	...
	public static class HibernateLoggingConfig {
		...
		private Level hibernateRootLogLevel = Level.SEVERE;
