...
public class IngredientTxWriter extends AbstractItemWriter
{
  ...
  public void writeItems(final List<Object> list)
  {
    ...
    for (Object o : list)
    {
      ...
      List<Ingredient> resultList = getIngredientList(transaction);
  ...
  /**
   * Find all ingredients from the name of the ingredient in the transaction
   * @param transaction
   * @return A list of Ingredients with names matching transaction.name
   */
  private List<Ingredient> getIngredientList(IngredientTransaction transaction) {
